...
export const patientRegister = catchAsyncErrors(async (req, res, next) => {
  ...
  //   checks by email in db if the user already exists
  let user = await User.findOne({ email });
  if (user) {
    return next(new ErrorHandler("User already registered", 400));
  }

  //   if user of same email not present then create it in the db
...
export const logoutAdmin = catchAsyncErrors(async (req, res, next) => {
  // on logoutAdmin, set adminToken value to "", ie remove it
  res
    .status(200)
    .cookie("adminToken", "", {
      httpOnly: true,
...
export const addNewDoctor = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!cloudinaryResponse || cloudinaryResponse.error) {
    console.error(
      "Cloudinary Error: ",
      cloudinaryResponse.error || "unknown cloudinary error occurred"
    );
  }
